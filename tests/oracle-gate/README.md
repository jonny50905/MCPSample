...
| no-todo | 不寫 todo 就 connect → 被擋 → todowrite → connect → task | 第一個 connect 的 before 列 decision=block、note=TODO_FIRST:NO_TODO、todoBlocked=1，transcript 該件 error 含 PS_TODO_FIRST_REQUIRED 與 todowrite 指示；假 MCP 沒收到那次 connect（mcp 順序 connect、run_sql）；順序＝被擋 connect → 含第 0 步的 todowrite → connect allow → READY → task |

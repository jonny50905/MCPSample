...
| 檔 | 用途 | 跑法 |
|---|---|---|
| `unit.test.mjs` | 狀態機單元測試（假 client；7 組：DB 判定／list→connect 順序／NOT_CONNECTED 退回與 disconnect／observe 模式／MCP 未掛載退讓與祖先 READY／連線 epoch（同行程別 session、跨行程改檔、disconnect）／mcp 狀態不快取） | `node --test tests/oracle-gate/unit.test.mjs` |
...
| epoch | list → connect → 假模型改寫 connection-epoch.json（模擬另一行程 connect）→ task | 被擋且 note=shared connection changed、模型收到「共用連線已被其他 session」；重做 list→connect 後放行 |

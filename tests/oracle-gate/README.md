...
| 檔 | 用途 | 跑法 |
|---|---|---|
| `unit.test.mjs` | 狀態機單元測試（假 client；5 組：DB 判定／list→connect 順序／NOT_CONNECTED 退回與 disconnect／observe 模式／MCP 未掛載退讓與祖先 READY） | `node --test tests/oracle-gate/unit.test.mjs` |
...
| `mock-model.mjs` | 假 OpenAI 相容模型（SSE 串流），依對話劇本決定下一個工具呼叫；`MOCK_MODEL_SCENARIO`：task-first／connect-first／compliant／stubborn／nodb | 由 e2e 啟動 |

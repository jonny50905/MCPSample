...
//     shared-switch  list → connect → （模擬另一個行程把共用連線切到別的名字：改寫 MOCK_STATE_FILE）→ task → 被擋（共用連線已變）
//                    → 依訊息只再 connect → task → 收尾
//     connect-fail   task → 被擋 → list → connect（失敗）→ task → 被擋 → connect（失敗）→ task → 閘門退讓 → 收尾
//   劇本以「最後一則 user 訊息之後」的工具呼叫為準（同 session 多 turn 時每 turn 重新走劇本）
//   MOCK_MODEL_LOG        每次請求追加一行 JSON
//   MOCK_STATE_FILE       shared-switch 情境要改寫的檔（plugin 的 connection-state.json）
...
function primaryNext(calls) {
  ...
  switch (scenario) {
    ...
    case "shared-switch": {
      ...
      if (n === 1) return CONNECT(pickConnection(calls[0].result))
      if (n === 2) {
        // 模擬另一個 OpenCode 行程剛把共用連線切到別的名字：直接改寫共用狀態檔
        const f = process.env.MOCK_STATE_FILE
        if (f) { try { fs.writeFileSync(f, JSON.stringify({ name: "OTHER_DB", changed: "foreign-" + Date.now(), ts: new Date().toISOString(), by: { tool: "oracleMCP_connect", sessionID: "ses_other", pid: 0 } })) } catch {} }
        return TASK("ps-ui-flow")
      }
      // 被擋的訊息說「只需再 connect、不必重做 list」

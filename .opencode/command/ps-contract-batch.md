...
3. 缺料才委派，且一個單位至多 2 個委派、會查 oracleMCP 的同時 ≤ 3、第一個先單獨派再並行（查完不得 disconnect）：
   ...
   - screen 缺導覽入口／technicalMenu → 委派 @ps-ui-flow：`[任務] Component <名> 的 Classic 導覽入口（cookbook §2k-C canonical；每個 CLASSIC_VISIBLE=1 的列一列，標入口型與可見性 CLASSIC_NAV_VISIBLE；CLASSIC_VISIBLE=0 的列只回 gaps）與 technicalMenuLocation（§2e）`

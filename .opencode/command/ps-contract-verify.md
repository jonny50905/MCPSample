...
- **一個單位一個委派**（manifest 的「## 單位 n」＝一個委派＝一個收據檔），禁止把多個單位或多個樣板族塞進同一委派；同時 ≤ 3、連線已在你的第 0 步建好（agent 規則：開場 list_connections → connect，無條件；oracleMCP 連線全域單例；查完不得 disconnect；subagent 回 BLOCKED＋NOT_CONNECTED 時你重連一次、重派一次）。
- 委派模板（只傳事實，不貼 NN 內容）：

...
**委派顆粒度與併發上限（L67）**：
- **一個委派只做一件事**：一個檔的一個任務。**禁止把多個檔、多個 A 項區間
  或多個任務塞進同一個委派**——實測（2026-08）單一 subagent 被交付 3 個任務
  後靜止、零輸出零錯誤。顆粒度過粗是已知的卡死來源。
- **併發上限依「這個委派會用哪個 server」分兩級（L83）**：
  · **會查 DB 的委派：上限 3**——SQL 型證據重跑、任務 C 的資料角度反查、
    metadata 類。SOP-12 實測值，oracleMCP 是**單工單一連線**，再多會排隊逾時。
  · **純 ES＋Source 的委派：上限 6**——任務 A 的 ChunkId 解引用、多數任務 B。
    **它們完全不碰 oracleMCP，不受那條限制**（6＝2026-08 實際運行過的值）。
  同時派出總數不超過 6，其中會查 DB 的不超過 3。
  **判準是「這個委派實際會呼叫哪個 server」，不是「它屬於哪個任務類型」**——
  同樣是任務 A，純 chunk 解引用不碰 DB，SQL 型證據重跑會碰。
- **不要改成全循序**：subagent 卡死是機率性的（L34 抽樣事故），循序會讓
  單點故障變成整輪停擺；適度並行時一個卡住只損失那一個委派，其餘照常完成，
  部分進度保得住。
...
- FAIL(STALE_DATA)：SQL 數值時效過期——重跑 cookbook 樣板取新值、
  更新文件中的數字即可，不必重做分析。
- FAIL(ID_RELINK)：id 失聯但稽核**已經重找到了**——舊值與新 id 都在
  `90-audit.md` 明細該列的「處置」欄（`換 id：<舊 UUID> → <新 UUID>`）。
  **read 該檔，把舊 UUID 的所有出現處換成新 UUID**（同一 chunk 常被多列
  引用，漏換等於下輪再開一次單），只用新 id 呼叫一次 `get_chunks_details`
  驗貨（ChunkText 需含原 quote）——**不必重做二次定位**，稽核已經付過了。
  舊 UUID 在該檔找不到＝該列已被改過或刪除，記收據跳過，不得硬塞。
  這一類 lint 也會自動開成 `[回灌]` 手術工單，多半在你拿到 A 項前就被套用完了。
...
4. **後寫記分卡**：依 `.opencode/peoplesoft/report-templates/audit-template.md`
   ...
   **明細就是下一輪的工單，而且 lint 會機械解析「處置」欄**——
   `FAIL(ID_RELINK)` 列一律寫成 `換 id：<完整舊 UUID> → <完整新 UUID>`
   （舊值取 auditor 的 `ref`、新值取 `newRef`，**兩個都要完整 36 字元、
   逐字複製，不得縮寫**）。一欄自足是刻意的：修復者要靠**舊值**在文件裡
   定位是哪一列，只給新 id 它不知道要換哪裡。
   `LINE_DRIFT` 寫 `更新行號 → <新行號>`；`FAIL(STALE_DATA)` 寫
   `更新數值 → <新值>`；其餘（NOT_FOUND／FABRICATED／WRONG_KIND…）才寫
   「回灌補查」。**稽核已經查到的答案不要丟掉**——A 行只有計數、傳不了這些，
   丟掉等於下一輪全量重驗要再付一次同樣的檢索成本。
...
- **代碼語意**：物件名前綴、代碼、縮寫的意義以
  ...
  為準。**表中已列的不得改寫或另作解釋**（例：AZ 是亞利桑那州，不准讀成澳洲）。
  **表中沒有的可以推測，不要卡住**——寫出最合理的展開即可，但該處標
  「(推測)」、gaps 記一行，管理者糾正後回填該檔。

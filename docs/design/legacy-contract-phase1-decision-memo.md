...
- `/ps-contract-verify <領域>`：第一動作 read `contract-parts/verify-manifest.txt`；每單位（OBJ／FLD-a-b／RQ-n）一個委派 @ps-metadata-flow 跑 cookbook §7 樣板（同時 ≤ 3、首個先單獨派，L109）→ 寫 `verify-<RECNAME>-<單位>.md`（「## 查詢」表：單位｜樣板｜SQL｜關鍵列｜狀態；FLD 加「## 欄位」表、OBJ 加「## 物件」表）；模型不判 PASS／FAIL。

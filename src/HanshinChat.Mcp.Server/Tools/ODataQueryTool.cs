...
public class ODataQueryTool
{
    ...
    [McpServerTool(Name = "query_odata"), Description(
        "Run a generic read-only OData v4 query against HanshinChat. " +
        "Allowed entities: Agents, Conversations, Messages, NluCallLogs. " +
        "Returns the raw OData JSON response (an object with @odata.context and a 'value' array, or a single entity).")]

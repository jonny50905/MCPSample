...
1a. oracleMCP 根因修正＋#23 research 債＋#24 導覽路徑（2026-09-03～04；**從 handover 分支搬時**用本表；若步驟 1 的 5 檔尚未搬，兩批一起搬；manifest 只搬最新）：
   ...
   | 檔案 | 新增／修改 | 行數 | 備註 |
   |---|---|---|---|
   | `.opencode/peoplesoft/oracle-query-cookbook.md` | 修改（生命週期＋平行規則＋#24 §2e／§4 正名、§2k＋L112 清理＋#27 Classic canonical＋#28 連線歸主 agent＋#28 工具名兩種拼法＋底線拼法＋開場無條件 connect＋list→connect 順序） | 636 | handover 版本行數 |
   ...
   | `.opencode/agent/ps-orchestrator.md` | 修改（#24 路徑類問題委派＋作答紀律＋清除一次只准一個殘留、歸戶分流＋L112 清理＋#27 Classic canonical＋#28 連線歸主 agent＋#28 工具名兩種拼法＋底線拼法＋開場無條件 connect＋list→connect 順序） | 186 | handover 版本行數 |
   ...
   | `.opencode/peoplesoft/customization-profile.yaml` | 修改（navigation 區塊：Classic 導覽環境事實＋oracle.connectionName＋底線拼法＋list→connect 順序） | 101 | 管理者依 SOP-20 核對 portal／labelLanguage 後把 verified 設 true；handover 版本行數 |
   ...
   | `.opencode/peoplesoft/test-scenarios.md` | 修改（§7 oracleMCP 連線擁有權手動回歸 R1～R7＋R0＋底線拼法＋開場無條件 connect＋list→connect 順序） | 672 | 公司機手動跑；結果記 applied.md L114；handover 版本行數 |
   ...
   | `.opencode/command/ps-audit-batch.md` | 修改（oracleMCP 委派 ≤ 3、首個先單獨派→#28 改主 agent connect＋L112 清理＋#28 連線歸主 agent＋開場無條件 connect＋list→connect 順序） | 109 | handover 版本行數 |
   | `.opencode/command/ps-audit.md` | 修改（≤ 3、首個先單獨派→#28 改主 agent connect＋L112 清理＋#28 連線歸主 agent＋開場無條件 connect＋list→connect 順序） | 72 | handover 版本行數 |
...
1. 搬 35 檔（功能分支＝contract 線 11 檔 ∪ oracleMCP 根因修正 11 檔 ∪ #23 三檔 ∪ #24 八檔，重疊 3 檔；#24 另改 contract 線 5 檔（vocabulary／fragments／lib／test-contract／ps-contract-batch）；核對欄：行數＝編輯器總行數，允許 ±1 行尾差異）：
   ...
   | `.opencode/peoplesoft/oracle-query-cookbook.md` | 修改（連線生命週期改版＋只加 §7＋#24 §2e／§4 正名、§2k＋L112 清理＋#27 Classic canonical＋#28 連線歸主 agent＋#28 工具名兩種拼法＋底線拼法＋開場無條件 connect＋list→connect 順序） | 730 | 生命週期 L48 起；§2k 自 L291 起、§7 自 L541 起，樣板全標待公司機驗證 |

...
**目前優先入口（2026-09-20）**：管理者要的不是先手填私有 pack，而是輸入一個或多個 Component，
產生供獨立 LLM 以現代架構重建核心功能的詳細 Spec。已在本分支直接新增 `/ps-spec`／`ps-spec-author`／
`ps-spec-build.ps1`，10 個深度主題、分頁研究與獨立 session 覆核、使用鏈範圍控制、繁體中文與原技術名、
缺口定向重試、可續跑及不可覆寫世代；SOP-25 是現行操作，SOP-24 私有 pack 僅保留給固定模板情境。
...
**再追記（2026-09-07，管理者定案）**：公司機 OpenCode 的工具名就是底線版——全樹一律寫 `list_connections`／`sql_run`，連字號拼法全部移除、不再兩種都開（三個主 agent tools 表只留 `oracleMCP_list_connections`；cookbook 第 0 步列工具名；SOP-12 再追記；test-scenarios §7；情境 31 加守衛擋連字號拼法回流）。
...
per-session 狀態機 NEED_LIST→NEED_CONNECT→READY、每則訊息重置；`task` 目標為會查 DB 的 subagent（agent 檔 tools 表 sql_run 為開）
...
**oracleMCP 未掛載改為擋**（訊息走 ORACLE_MCP_DOWN 協定，不再有環境層退讓）、四個 DB subagent 的 Oracle 工具改允許清單（只開 sql_run）、
...
NOT_CONNECTED 不作廢新世代；task after 解析報告（reportStatus／blockedReason／childSessionID）、sql_run after 記 ok；analyzer 完成＝報告 COMPLETE
且子 session sql_run 成功。驗證：單元 18、e2e 17 情境（新增 wrong-target／not-configured／reconnect-fail）、analyzer 對真 export 一致、
...
**再追記（同日，工具名更正）**：管理者核對 /mcp——oracleMCP 的查詢工具實名是 `oracleMCP_sql_run`，全樹自 ae72d49（2026-09-07，issue #28
...
在 pwsh 7.6 全綠；**公司機 PS 5.1 仍未實跑**。先前 memo 與守衛 regex 曾出現一個非合成的 TW_ 物件名（6466d07／c441e04），本輪已移除；管理者已確認那是憑空造的假名、
系統裡沒有這個物件，也從未提供任何機密給 session，所以不需要改寫歷史。搬運清單見 §1 步驟 1d。
...
**追記（2026-09-18，公司機首跑與 Template 綁定）**：公司機 PS 5.1 首跑回報 test-knowledge 2 個、test-supplemental 12 個 FAIL，
全是測試框架問題（`Out-String` 折行與 CRLF；子行程 `powershell.exe -File` 沒帶 -ExecutionPolicy Bypass 被執行原則擋），
改同行程呼叫、逐筆 `[string]` 取字；順帶抓到「陣列 splat 對 script 只走位置繫結」的陷阱。Spec 需求包的 Template 綁定改為
**標題綁定**（`bindingMode: headings`）：Template 副本原樣放進 pack、slot 綁章節標題＋章節內原生 `{{…}}` 佔位符、文件參數用
`placeholders` 表、`-InitPack` 產骨架；`{{slot:Sxx}}` 只給合成範例。另修 render 表格分隔列被併進表頭的 bug。
公司機接入步驟見 SOP-24 步驟 1；搬運見 §1 步驟 1d（pack-c 兩檔新增）。
...
1d. issue #33～#36（2026-09-17）＋對抗審查修正（同日；1／1a／1b／1c 尚未搬的一起搬，manifest 只搬最新）——全部照貼即可，沒有必刪檔；
    `.gitignore` 不搬，但**內部 git 要自己加四行**（`docs/ps-research/knowledge/`、`docs/ps-research/*/supplemental-parts/`、`.ps-private/`、
    `.ps-runtime/`）；公司機 PS 5.1 首跑順序：`ps-fs-doctor` → `test-ps51-static` → `test-knowledge` → `test-supplemental` → `test-spec` →
    `test-auto-loop`，只回報結論碼與 PASS／FAIL：

   | 檔案 | 新增／修改 | 行數 | 備註 |
   |---|---|---|---|
   | `.opencode/agent/ps-orchestrator.md` | 修改 | 228 | 第 3／7／8 步：知識層契約、封閉標籤（含 BLOCKED／wiki 草稿／過期）、`[|] 物件 [|]` |
   | `.opencode/agent/ps-spec-worker.md` | 新增 | 78 | tools 補關；permission 縮到 attempts 子樹（`*` 開頭） |
   | `.opencode/command/ps-correct.md` | 修改 | 27 | 提示 -Rebuild |
   ...
   | `.opencode/peoplesoft/SOP.md` | 修改 | 876 | SOP-22～24（含 slot 等待、SUPP1-3-03／1-06、SPEC 9-07／7-22／4-07／3-07） |
   | `.opencode/peoplesoft/knowledge-retrieval-contract.md` | 新增 | 124 | pattern 字元類別、limit 直接用、首行前綴自檢、wiki 路徑、有效性 → 標籤 |
   | `.opencode/peoplesoft/lessons/applied.md` | 修改 | 3486 | L119～L123 |
   ...
   | `.opencode/peoplesoft/spec/examples/pack-c/pack.json` | 新增 | 33 | headings 模式範例（Template 原樣＋原生佔位符） |
   | `.opencode/peoplesoft/spec/examples/pack-c/template-bound.md` | 新增 | 28 | 合成模板：固定章節、{{PRODUCT_NAME}}、引導式佔位符 |
   | `.opencode/peoplesoft/spec/generic.manifest.json` | 新增 | 76 | `-Doctor -WriteGenericManifest` 重生（14 檔） |
   | `.opencode/peoplesoft/spec/pack.schema.json` | 新增 | 85 |  |
   | `.opencode/peoplesoft/spec/support-codes.md` | 新增 | 168 | KNOW1／SUPP1／SPEC1 結論碼（新增 SUPP1-1-06／3-03、SPEC1-3-07／4-07／7-22／9-07） |
   | `.opencode/peoplesoft/spec/troubleshooting-matrix.md` | 新增 | 33 | 責任分工與不變量 |
   | `.opencode/peoplesoft/supplemental-contract.md` | 新增 | 100 | 第 0 步順序、空表、格內不得 `|`、NO_EVIDENCE 範例 |
   | `.opencode/peoplesoft/test-scenarios.md` | 修改 | 732 | I2 改寫、J7、7b |
   | `.opencode/skills/ps-business-discovery/SKILL.md` | 修改 | 187 | 索引定位 |
   | `.opencode/skills/ps-business-explain/SKILL.md` | 修改 | 116 | 來源標籤封閉集合 |
   ...
   | `scripts/ps-spec-lib.ps1` | 新增 | 2666 | Spec 引擎 lib（派工重定位、live grade 7-22、PARTIAL_SPLIT 4-07、WRITE_DEFERRED 3-07、parser 寬鬆） |
   | `scripts/ps-spec.ps1` | 新增 | 306 | `-ValidatePack/-Plan/-Run/-Render/-Gate/-Doctor`；-RuntimeRoot 驗證 9-07 |
   ...
   | `scripts/ps-transfer-manifest.json` | 修改 | 531 | 最後搬；fs-doctor 應報 86 檔一致（commit 欄＝產生時 HEAD，早一步屬預期） |
   | `scripts/tests/test-auto-loop.ps1` | 修改 | 1022 | 情境 34（.gitignore 缺檔跳過、提示常值閘門、worker 工具與圍欄、slot、契約 pattern）；情境 34 圍籬單元測試與中文目錄快照回歸 |
   ...
   | `scripts/tests/test-spec.ps1` | 新增 | 1022 | Spec 測試組（情境 11～20 新增） |
   | `scripts/tests/test-supplemental.ps1` | 新增 | 524 | 補研究測試組（情境 9：intake／callee／wiki／DOMAIN_MISSING／空表）；合併痕跡與 request／result 檔驗證 |

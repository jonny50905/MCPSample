...
1. 搬 5 檔（核對欄：行數＝編輯器總行數，允許 ±1 行尾差異）：
   ...
   | `scripts/tests/test-auto-loop.ps1` | 新增（新目錄 `scripts\tests\`＋情境 28＋情境 29＋情境 30） | 627 | 存 UTF-8 with BOM；公司機以 `pwsh -NoProfile -File` 跑 |
...
1a. oracleMCP 根因修正＋#23 research 債＋#24 導覽路徑（2026-09-03～04；**從 handover 分支搬時**用本表；若步驟 1 的 5 檔尚未搬，兩批一起搬；manifest 只搬最新）：
   ...
   | `.opencode/agent/ps-auditor.md` | 修改（tools 硬性 deny＋規則＋#24 三個 FAIL 原因＋L112 清理＋#27 Classic canonical） | 257 | handover 版本行數 |
...
1. 搬 34 檔（功能分支＝contract 線 11 檔 ∪ oracleMCP 根因修正 11 檔 ∪ #23 三檔 ∪ #24 八檔，重疊 3 檔；#24 另改 contract 線 5 檔（vocabulary／fragments／lib／test-contract／ps-contract-batch）；核對欄：行數＝編輯器總行數，允許 ±1 行尾差異）：
   ...
   | `.opencode/agent/ps-auditor.md` | 修改（tools 硬性 deny＋規則＋#24 三個 FAIL 原因＋L112 清理＋#27 Classic canonical） | 257 | L109 |

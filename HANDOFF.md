...
不動 ps-auto-loop／lint／ps-deep-research／畢業門；`scripts/tests/test-contract.ps1` K1～K10 共 113 判定
...
**功能分支追記（2026-09-04，issue #24 contract 線）**：`menuPath` kv 改 `technicalMenu`（PSMENUITEM 三欄、非導覽路徑）、`## 導覽` 表加「入口型」「可見性」兩欄、NAV ID 含入口型（`NAV.<COMP>.<FROM>.<TO>.<KIND>.<ENTRYTYPE>`）、四元組唯一與禁填 AUTHORIZED_FOR_CONTEXT 兩條不變量、UNKNOWN_VISIBILITY 出 debt 不立 gate；`ContractSchemaVersion` 1→2、`vocabularyVersion` 1→2（既有 contract JSON／spec／gate 收據全部重生屬預期）。test-contract 106 判定、test-auto-loop 全 PASS。**審查補強（同日）**：lib 加 technicalMenu 形狀檢查（含 `>`＝INVALID）、Portal 入口列可見性不得 NOT_APPLICABLE、來源／目標須自然鍵（ID 消毒後不撞名）、spec 固定印「其他導覽 surface 未盤查」列＋`alternateSurfaces｜NOT_INSPECTED` debt（不擋 tier 2）；test-contract 111 判定。**追記（issue #27）**：vocabulary 3（CLASSIC_NAV_VISIBLE）、lib 讀 profile navigation.surfaces（CLASSIC_ONLY 不出 alternateSurfaces 債、spec 印 NOT_APPLICABLE）、ps-contract.ps1 帶入；test-contract 113 判定。
...
1. 搬 5 檔（核對欄：行數＝編輯器總行數，允許 ±1 行尾差異）：
   ...
   | `.opencode/agent/ps-audit-orchestrator.md` | 修改（L112 清理＋#28 連線歸主 agent＋#28 工具名兩種拼法） | 144 | 備用、未掛載，但 manifest 要對 |
   | `scripts/tests/test-auto-loop.ps1` | 新增（新目錄 `scripts\tests\`＋情境 28＋情境 29＋情境 30＋情境 31＋情境 31 拼法守衛） | 663 | 存 UTF-8 with BOM；公司機以 `pwsh -NoProfile -File` 跑 |
...
1a. oracleMCP 根因修正＋#23 research 債＋#24 導覽路徑（2026-09-03～04；**從 handover 分支搬時**用本表；若步驟 1 的 5 檔尚未搬，兩批一起搬；manifest 只搬最新）：
   ...
   | 檔案 | 新增／修改 | 行數 | 備註 |
   |---|---|---|---|
   | `.opencode/peoplesoft/oracle-query-cookbook.md` | 修改（生命週期＋平行規則＋#24 §2e／§4 正名、§2k＋L112 清理＋#27 Classic canonical＋#28 連線歸主 agent＋#28 工具名兩種拼法） | 637 | handover 版本行數 |
   | `.opencode/agent/ps-ui-flow.md` | 修改（生命週期＋#24 導覽職責＋L112 清理＋#27 Classic canonical＋#28 連線歸主 agent＋#28 硬規則段去舊流程） | 142 | handover 版本行數 |
   | `.opencode/agent/ps-metadata-flow.md` | 修改（生命週期＋#24 授權≠導覽＋L112 清理＋#28 連線歸主 agent＋#28 硬規則段去舊流程） | 110 | handover 版本行數 |
   | `.opencode/agent/ps-ae-flow.md` | 修改（生命週期＋#28 連線歸主 agent＋#28 硬規則段去舊流程） | 92 | handover 版本行數 |
   ...
   | `.opencode/agent/ps-orchestrator.md` | 修改（#24 路徑類問題委派＋作答紀律＋清除一次只准一個殘留、歸戶分流＋L112 清理＋#27 Classic canonical＋#28 連線歸主 agent＋#28 工具名兩種拼法） | 177 | handover 版本行數 |
   ...
   | `.opencode/peoplesoft/customization-profile.yaml` | 修改（navigation 區塊：Classic 導覽環境事實＋oracle.connectionName） | 101 | 管理者依 SOP-20 核對 portal／labelLanguage 後把 verified 設 true；handover 版本行數 |
   ...
   | `.opencode/peoplesoft/test-scenarios.md` | 修改（§7 oracleMCP 連線擁有權手動回歸 R1～R7＋R0） | 671 | 公司機手動跑；結果記 applied.md L114；handover 版本行數 |
   ...
   | `scripts/ps-doc-lint.ps1` | 修改（#24 兩條導覽規則＋[導覽] 工單，美工類＋#27 Classic canonical） | 1612 | 存 UTF-8 with BOM；handover 版本行數 |
   ...
   | `.opencode/agent/ps-deep-research.md` | 修改（三處 ≤ 3＋#24 導覽入口填法＋清除一次一個殘留＋L112 清理＋#27 Classic canonical＋#28 連線歸主 agent＋#28 工具名兩種拼法） | 510 | handover 版本行數 |
   | `.opencode/agent/ps-audit-orchestrator.md` | 修改（≤ 3、首個先單獨派→#28 改主 agent connect＋L112 清理＋#28 連線歸主 agent＋#28 工具名兩種拼法） | 144 | handover 版本行數 |
   | `.opencode/peoplesoft/SOP.md` | 修改（只加 SOP-12 補述＋SOP-13 tier 1 門＋SOP-19＋SOP-20＋SOP-12 追記＋SOP-12 再追記） | 649 | handover 版本行數 |
   | `.opencode/peoplesoft/lessons/applied.md` | 修改（只加 L109＋L110＋L111＋L109 追記＋L112＋L113＋L114＋L114 追記） | 3056 | handover 版本行數 |
   ...
   | `scripts/tests/test-auto-loop.ps1` | 修改（情境 27＋情境 28＋情境 29＋情境 30＋情境 31＋情境 31 拼法守衛） | 663 | 存 UTF-8 with BOM；handover 版本行數 |
...
1. 搬 35 檔（功能分支＝contract 線 11 檔 ∪ oracleMCP 根因修正 11 檔 ∪ #23 三檔 ∪ #24 八檔，重疊 3 檔；#24 另改 contract 線 5 檔（vocabulary／fragments／lib／test-contract／ps-contract-batch）；核對欄：行數＝編輯器總行數，允許 ±1 行尾差異）：

   | 檔案 | 新增／修改 | 行數 | 備註 |
   |---|---|---|---|
   | `.opencode/peoplesoft/legacy-contract-vocabulary.md` | 新增（#24 navigationEntryType／navigationVisibility＋L112 清理＋#27 Classic canonical） | 422 | 封閉值域單一真相（vocabularyVersion 2；#24 升版） |
   ...
   | `.opencode/peoplesoft/oracle-query-cookbook.md` | 修改（連線生命週期改版＋只加 §7＋#24 §2e／§4 正名、§2k＋L112 清理＋#27 Classic canonical＋#28 連線歸主 agent＋#28 工具名兩種拼法） | 731 | 生命週期 L48 起；§2k 自 L291 起、§7 自 L541 起，樣板全標待公司機驗證 |
   | `.opencode/agent/ps-ui-flow.md` | 修改（生命週期＋#24 導覽職責＋L112 清理＋#27 Classic canonical＋#28 連線歸主 agent＋#28 硬規則段去舊流程） | 142 | L109 |
   | `.opencode/agent/ps-metadata-flow.md` | 修改（生命週期＋#24 授權≠導覽＋L112 清理＋#28 連線歸主 agent＋#28 硬規則段去舊流程） | 110 | L109 |
   | `.opencode/agent/ps-ae-flow.md` | 修改（生命週期＋#28 連線歸主 agent＋#28 硬規則段去舊流程） | 92 | L109 |
   ...
   | `.opencode/agent/ps-orchestrator.md` | 修改（#24 路徑類問題委派＋作答紀律＋清除一次只准一個殘留、歸戶分流＋L112 清理＋#27 Classic canonical＋#28 連線歸主 agent＋#28 工具名兩種拼法） | 177 |  |
   ...
   | `.opencode/peoplesoft/customization-profile.yaml` | 修改（navigation 區塊：Classic 導覽環境事實＋oracle.connectionName） | 101 | 管理者依 SOP-20 核對 portal／labelLanguage 後把 verified 設 true；oracle.connectionName 本機回填（之後 fs-doctor 報此檔 M 屬預期） |
   | `.opencode/peoplesoft/subagent-report-contract.md` | 修改（#24 硬規則 3a＋兩個選填陣列＋L112 清理＋#27 Classic canonical＋#28 連線歸主 agent） | 169 |  |
   | `.opencode/peoplesoft/test-scenarios.md` | 修改（§7 oracleMCP 連線擁有權手動回歸 R1～R7＋R0） | 671 | 公司機手動跑；結果記 applied.md L114 |
   ...
   | `scripts/ps-doc-lint.ps1` | 修改（#24 三型導覽規則＋[導覽] 工單，美工類；審查補強＋#27 Classic canonical） | 1612 | 存 UTF-8 with BOM |
   ...
   | `.opencode/agent/ps-deep-research.md` | 修改（三處 ≤ 3＋#24 導覽入口填法＋清除一次一個殘留＋L112 清理＋#27 Classic canonical＋#28 連線歸主 agent＋#28 工具名兩種拼法） | 510 |  |
   | `.opencode/agent/ps-audit-orchestrator.md` | 修改（≤ 3、首個先單獨派→#28 改主 agent connect＋L112 清理＋#28 連線歸主 agent＋#28 工具名兩種拼法） | 144 |  |
   | `.opencode/peoplesoft/SOP.md` | 修改（只加 SOP-12 補述＋SOP-13 門＋SOP-18＋SOP-19＋SOP-20＋SOP-12 追記＋SOP-12 再追記） | 701 | SOP-18 自 L604 起；SOP-19 自 L656 起 |
   | `.opencode/peoplesoft/lessons/applied.md` | 修改（只加 L108＋L109＋L110＋L111＋L109 追記＋L112＋L113＋L114＋L114 追記） | 3112 | L108 自 L2879、L109 自 L2935 起；L111 自 L3000 起 |
   ...
   | `scripts/tests/test-auto-loop.ps1` | 修改（情境 27＋情境 28 共 17 判定＋情境 29＋情境 30＋情境 31＋情境 31 拼法守衛） | 663 | 存 UTF-8 with BOM |
...
- **測試**：`pwsh -File scripts/tests/test-auto-loop.ps1`（28 個真實函式 AST 抽取、情境 27 含 #23、
  ...
  （K1～K10、113 判定、不需模型；改 ps-contract*.ps1／vocabulary／fragments 後必跑）。
